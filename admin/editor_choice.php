...
<?php include('nav.php') ?>
  ...
  <tbody>
      ...
      $sql = "SELECT * FROM blogs where editor_choice = ?";
      $stmt = mysqli_prepare($con, $sql);
      if($stmt){
        $one = 1;
        $result = mysqli_stmt_bind_param($stmt, "s", $one);
...
<tr>
      ...
      <td><a class="btn btn-danger" onclick="getUpdate('<?php echo $id; ?>')">Delete</a></td>
...
function getUpdate(id){
  if (confirm("Are you sure!")) {
    $.ajax({type: "POST",
         url: "post.php",  
          data:{update:'update',id:id, field:'editor_choice', update_data:0},  
